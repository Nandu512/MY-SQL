INSERT INTO tcenter(id,name,course,fees_paid,status_paid)
VALUES(1, 'Alice', 'Web Development', 5000, 'Inactive'),
(2, 'Bob', 'Data Science', 7000, 'Inactive'),
(3, 'Charlie', 'UI/UX Design', 4000, 'Active');
SELECT*FROM tcenter
WHERE fees_paid>5000;
UPDATE tcenter
SET status_paid='Active'
WHERE course='Web Development'
UPDATE tcenter
SET fees_paid=fees_paid+1000
WHERE course='Data Science'
UPDATE tcenter
SET status_paid='Inactive',fees_paid=fees_paid-500
WHERE id=3
DELETE FROM tcenter
WHERE id=2
DELETE FROM tcenter
WHERE status_paid='Inactive'
